...
export const dashboardService = {
  ...
  async getStats() {
    await delay()
    const denuncias   = mockStorage.get('denuncias')   ?? MOCK_DENUNCIAS
    const monitoreos  = mockStorage.get('monitoreo_denuncias') ?? MOCK_MONITOREO_DENUNCIAS
    const acciones    = mockStorage.get('acciones')    ?? MOCK_ACCIONES
    const usuarios    = mockStorage.get('usuarios')    ?? MOCK_USUARIOS
